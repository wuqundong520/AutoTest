  ...
  Tree.prototype.tree = function(elem, children){
    ...
    layui.each(nodes, function(index, item){
      ...
      var li = $(['<li '+ (item.spread ? 'data-spread="'+ item.spread +'"' : '') +'>'
        //展开箭头
        ,function(){
          return item.parented ? '<i class="layui-icon layui-tree-spread">'+ (
            item.spread && hasChild ? icon.arrow[1] : icon.arrow[0]
          ) +'</i>' : '';
        }()
        ...
        //节点
        ,function(){
          return '<a id="' + item.id + '" href="'+ (item.href || 'javascript:;') +'" '+ (
            options.target && item.href ? 'target=\"'+ options.target +'\"' : ''
          ) +'>'
          + ('<i class="layui-icon layui-tree-'+ (item.parented ? "branch" : "leaf") + '" style="color:' + 
        	(!item.parented && item.invalid ? "#FF0000" : "" ) + ';">'
          + (
            (hasChild || item.parented) ? (
    ...
    //初始化之后的回调
    options.callback && options.callback(options.nodes);
  };
