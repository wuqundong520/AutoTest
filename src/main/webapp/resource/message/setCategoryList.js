...
var eventList = {
	"#add-object":function() {
		...
		layer_show("增加目录或测试集", editHtml, "550", editPageHeight.add, 1, function() {
			callback();
		});
		publish.init();			
	},
...
function opSetFolder(id) {
	layer.confirm('请选择你需要的操作:<br>点击其他位置关闭对话框', {
		...
		layer_show("编辑目录信息", editHtml, 550, editPageHeight.edit, 1);
		publish.init();	
		layer.close(index);
	});
}
...
function delSet (tip, id, callback) {
	layer.confirm(tip, {title:'警告', anim:5}, function(index) {	
				...
				$.post(SET_DEL_URL, {id:id}, function(json) {
					if (json.returnCode == 0) {
						createNodeTree();
