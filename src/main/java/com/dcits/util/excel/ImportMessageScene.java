...
public class ImportMessageScene {
	...
	public static Map<String, Object> importToDB (String path, Message message) {
		...
		SceneValidateRuleService sceneValidateRuleService = (SceneValidateRuleService) StrutsUtils.getSpringBean("sceneValidateRuleService");
	...
	public static Object[] validateInfo (MessageScene info, Message message) {
		...
		//设置默认验证规则
		if (StringUtils.isNotBlank(info.getValidateMethodStr())) {
			GlobalVariableService service = (GlobalVariableService) StrutsUtils.getSpringBean("globalVariableService");
			...
			if (variable == null) {
				tigs.append("没有找到Key名称为<strong>" + info.getValidateMethodStr() + "</strong>的关联验证模板,默认不设置默认验证规则;");				
			} else {
				try {
