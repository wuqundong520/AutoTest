...
public class ExportInterfaceInfo {
	...
	public static String exportDocuments (List<InterfaceInfo> infos, int type) throws Exception {
		...
		try {
			...
			outputStream = new FileOutputStream(new File(StrutsUtils.getProjectPath() + "/" + SystemConsts.EXCEL_FILE_FOLDER 
					+ "/" + fileName));
			
			wb.write(outputStream); 
			path = SystemConsts.EXCEL_FILE_FOLDER + "/" + fileName;
		} catch (Exception e) {
			// TODO: handle exception
			LOGGER.error("写Excel文件失败:文件路径" + StrutsUtils.getProjectPath() + "/" + SystemConsts.EXCEL_FILE_FOLDER  + "/" + fileName, e);
			throw e;
		} finally {
	...
	public static void writeSheet (InterfaceInfo info,  Workbook wb) {
		...
		String[][] titles = new String[][]{
				...
				new String[]{"请求地址_生产", info.getRequestUrlReal()},
				new String[]{"请求地址_测试", info.getRequestUrlMock()},
